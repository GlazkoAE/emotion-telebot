...
class ArousalModel:
    def __init__(
        ...
        # Init both models
        self.feature_extractor_sess = ort.InferenceSession(
            feature_extractor_path, providers=["CUDAExecutionProvider"]
        )
        self.arousal_model_sess = ort.InferenceSession(
            arousal_model_path, providers=["CUDAExecutionProvider"]
        )
    ...
    def extract_features(self, img):
        ...
        features = np.array(
            self.feature_extractor_sess.run(["avg_pool"], {"input_img": img})
        )
    ...
    def predict(self, image):
        ...
        predict = self.arousal_model_sess.run(
            ["dense_1"], {"input_features": self.features}
        )
